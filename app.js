...
function dropZoneNode(){ 
  ...
  div.innerHTML='<div class="dz-icon">＋</div>이미지 · 영상을 드래그하거나<br><span style="color:var(--accent)">클릭해서 파일 선택</span><br><span style="font-size:11px;color:var(--t4)">Ctrl+V 로 이미지 또는 URL 붙여넣기 가능</span>'; 
  return div; 
}
...
function renderBoard(){
  ...
  board.className=currentView+'-view'; board.innerHTML='';
  ...
  const chunkSize=currentView==='list'?80:40; let idx=0;
  function paintChunk(){
